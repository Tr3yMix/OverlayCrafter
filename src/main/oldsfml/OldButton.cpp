...
OldButton::OldButton(const ButtonType type, const sf::Vector2f& position, const sf::Vector2f& scale, const sf::Text& label, const Colors colors, const std::function<void()>& onClick): m_position(
...
OldButton::OldButton(const ButtonType type, const sf::Vector2f& position, const sf::Vector2f& scale,
...
void OldButton::draw(sf::RenderWindow& window) const {
    window.draw(m_shape);
    window.draw(*m_label);
}

bool OldButton::isHovering(const sf::RenderWindow& window, const sf::View& view) const {
...
void OldButton::handleEvent(const std::optional<sf::Event>& event, const sf::RenderWindow& window, const sf::View& view) {
...
void OldButton::setPosition(const sf::Vector2f& position) {
    m_shape.setPosition(position);
    m_label->setPosition(position + m_scale / 2.f);
}

void OldButton::setSize(const sf::Vector2f &scale) {
    m_scale = scale;
    m_shape.setSize(m_scale);
...
sf::FloatRect OldButton::getBounds() const {
    return m_shape.getGlobalBounds();
}

sf::Vector2f OldButton::getScale() const {
    return m_scale;
}
